...
public class S3App extends Canvas {
    ...
    public static final int SCREEN_X = 1920;
    public static final int SCREEN_Y = 1080;
...
//        }
    ...
    public void mouseRelease(int current_screen_x, int current_screen_y) {
        if (m_state == STATE_GAME) {
            ...
            // get selected units
            for (int i = Math.min(map_x_start, map_x_end); i < (Math.max(map_x_start, map_x_end)) + 1; i++) {
                for (int j = Math.min(map_y_start, map_y_end); j < (Math.max(map_y_start, map_y_end)) + 1; j++) {
